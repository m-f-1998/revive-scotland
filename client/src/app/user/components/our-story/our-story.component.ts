...
export class OurStoryComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit ( ): void {
    ...
    this.apiSvc.get ( "/api/content/our-story" ).then ( data => {
      ...
      if ( res.items?.length ) {
        this.ourStory.set ( {
          mediaUrl: res.mediaUrl || FALLBACK.mediaUrl,
          items: res.items
        } )
      }
    } ).catch ( ( ) => { /* keep fallback */ } )
