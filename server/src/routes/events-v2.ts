...
router.post ( "", async ( _req: Request, res: Response ) => {
  try {
    const client = await pool.query ( 'SELECT "id", "title", "description", "when", "longitude", "latitude", "showcase_image", "donation_requested", "donation_amount", "payment_required", "payment_amount" FROM events' )
    // convert longitude and latitude to location name if they exist
    return res.json ( client.rows.map ( async event => {
